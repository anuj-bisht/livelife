...
{ 
    ...
    public function ajaxNotificationData(){

        $id = Auth::user()->id;
        $data = Notification::select('title','message','created_at','message as notification_message','created_at as created')->where('user_id',$id)->get();
        return response()->json(["status"=>1,"message"=>"Notification sent","data"=>$data]);        
    }


    public function ajaxGetNotificationByUser(){
        $data = Notification::select('title','message','created_at','message as notification_message','created_at as created')->where('user_id','<>',0)->get();
        return response()->json(["status"=>1,"message"=>"Notification sent","data"=>$data]);        
    }
