...
function notificationData(){
	...
	jQuery.ajax({
		url: site_url+"/admin/notifications/ajaxNotificationData",
		method: 'post',
		data: {
			"_token": $('meta[name="csrf-token"]').attr('content')
		},
			...
			if(result.status){
                if(result.data.length>0){
                    $.each(result.data,function(index,value){
                        html += '<li><a href="#">';
                        html += '<div><i class="fa fa-comment fa-fw"></i>Title:'+value.title+'</div>';
                        html += '<div><i class="fa fa-comment fa-fw"></i>Message:'+value.message+'<span class="pull-right text-muted small">'+value.created_at+' ago</span></div>';
                        html += '</a></li>';				
                    })
                }else{
                    html += '<li><a href="#">';
